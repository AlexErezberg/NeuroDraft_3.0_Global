...
class NeuroDraftAssistant:
    ...
    def apply_gender(self, raw_data, gender, is_endo=False, lang='ru'):
        ...
        if lang == 'ru':
            is_fem = (gender == 'а')
            if is_fem:
                # Сначала обрабатываем "замедлен{g}" -> "замедлена"
                # Заменяем "ен{g}" на "ена" (для причастий) или "на" (для кратких прилагательных)
                # Самый безопасный путь для "замедлен{g}", "инертен{g}", "активен{g}"
                text = text.replace("ен{g}", "ена") # замедлен{g} -> замедлена
                text = text.replace("ый{g}", "ая").replace("ий{g}", "ая")
                text = text.replace("{g}", "а") # остальное
                
                # Словарные замены для тех, где корень меняется сильно
                f_map = {"инертен": "инертна", "активен": "активна", "пациент": "пациентка"}
                for m, f in f_map.items(): 
                    text = re.sub(rf"\b{m}\b", f, text)
            else:
                # Мужской род: восстанавливаем полные формы
                text = text.replace("ем{g}", "емый").replace("ый{g}", "ый").replace("ий{g}", "ий")
                text = text.replace("ен{g}", "ен") # замедлен{g} -> замедлен
                text = text.replace("{g}", "")
                text = text.replace("пациентка", "пациент")
    ...
    def run(self, code_str, pr_in="", t_in="", lang='ru', moca=None, mmse=None, gds=None, mri=""):
        try:
            ...
            # --- 4. ТЕГИ И НАДСТРОЙКИ (ФИНАЛЬНЫЙ ФИКС УБИЙЦЫ КЕННЕДИ) ---
            # Ищем базу тегов ГАРАНТИРОВАННО в корне библиотеки
            t_db = self.lib.get("tags", {})
...
PASSWORD = "1989" # <--- УБЕДИСЬ, ЧТО ОН ТУТ!
